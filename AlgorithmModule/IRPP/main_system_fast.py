...
class MCSSystem:
    ...
    def _assess_data_quality(self, worker_data: Dict[int, List[float]]) -> Dict[int, str]:
        ...
        # 步骤1: 识别高质量工人
        high_quality_workers = []
        all_worker_ids = list(worker_data.keys())

        print("识别高质量工人:")
        for worker_id in all_worker_ids:
            if worker_id in self.workers:
                # 计算信誉评分 - 使用现有的评分系统
                worker_triplet = self.workers[worker_id].get_triplet()
                workers_data = {worker_id: worker_triplet}
                results = self.scorer.score_multiple_workers(workers_data)

                if results:
                    reputation_score = results[0].reputation_score
                    print(f"  工人{worker_id}: 信誉评分={reputation_score:.4f}")

                    if reputation_score > self.reputation_threshold:
                        high_quality_workers.append(worker_id)

        print(f"高质量工人数量: {len(high_quality_workers)}, ID: {high_quality_workers}")

        # 步骤2: 判断是否使用单工人评估方法
        if len(high_quality_workers) > 8:
            # 只有当高质量工人数量超过8个时，才使用单工人评估方法
            print(f"高质量工人数量({len(high_quality_workers)})超过8个，使用单工人评估方法")

            # 随机选择8个高质量工人作为参考组
            import random
            reference_workers = random.sample(high_quality_workers, 8)
            print(f"从{len(high_quality_workers)}个高质量工人中随机选择8个作为参考组: {reference_workers}")

            # 准备参考数据
            reference_data = [worker_data[worker_id] for worker_id in reference_workers]
            ...
            for worker_id in all_worker_ids:
                target_data = worker_data[worker_id]

                # 如果当前工人在参考组中，需要从参考组中排除自己
                if worker_id in reference_workers:
                    current_reference_data = [worker_data[ref_id] for ref_id in reference_workers if
                                              ref_id != worker_id]
                    if len(current_reference_data) == 0:
                        # 如果排除自己后没有参考数据，跳过评估，标记为不确定
                        quality_results[worker_id] = DataQuality.UNCERTAIN.value
                        print(f"  工人{worker_id}: {DataQuality.UNCERTAIN.value} (无参考数据)")
                        continue
                else:
                    current_reference_data = reference_data

                try:
                    # 使用单工人评估方法
                    assessment_result = self.assessor.assess_single_worker(
                        target_worker_data=target_data,
                        other_workers_data=current_reference_data,
                        include_classification=True
                    )

                    total_assessment_time += assessment_result.execution_time

                    # 提取评估结果
                    result = assessment_result.results
                    ...
                    print(f"  工人{worker_id}: 评估失败 - {e}")
                    quality_results[worker_id] = DataQuality.UNCERTAIN.value

            print(f"单工人评估总耗时: {total_assessment_time:.4f}秒")
            print(f"平均每工人耗时: {total_assessment_time / len(all_worker_ids):.4f}秒")
            ...
            print(f"评估结果统计: 高质量={quality_counts[DataQuality.HIGH.value]}, "
                  f"不确定={quality_counts[DataQuality.UNCERTAIN.value]}, "
                  f"低质量={quality_counts[DataQuality.LOW.value]}")

            return quality_results

        else:
            # 如果高质量工人数量不超过8个，使用原来的多工人评估方法
            print(f"高质量工人数量({len(high_quality_workers)})不超过8个，使用原来的多工人评估方法")
...
def main():
    """主程序入口"""
    # 配置参数
    data_file_path = r"D:\py\IRPP\AlgorithmModule\Scene_3_Number_of_Workers_39.json"
